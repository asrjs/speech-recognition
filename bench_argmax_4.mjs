...
function argmaxAndSelectedLogProbsNoNullishCoalescing(logits, frameCount, vocabSize) {
  const frameIds = new Array(frameCount).fill(0);
  const selectedLogProbs = new Float32Array(frameCount);
  ...
  for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
    ...
    for (let vocabIndex = 0; vocabIndex < vocabSize; vocabIndex += 1) {
      // Memory rule: "In V8 (Node/Bun/Chrome), using the nullish coalescing operator (??) inside tight inner loops dealing with TypedArrays significantly degrades performance compared to using the non-null assertion operator (!). When bounds are known to be safe, prefer ! to preserve performance."
...
function runBench() {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    argmaxAndSelectedLogProbsNoNullishCoalescing(logits, frameCount, vocabSize);
  }
