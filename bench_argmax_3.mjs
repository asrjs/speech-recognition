...
function argmaxAndSelectedLogProbsOptimized(logits, frameCount, vocabSize) {
  ...
  for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
    ...
    frameIds[frameIndex] = bestId;
    selectedLogProbs[frameIndex] = -Math.log(expSum || 1); // This simplifies mathematically! bestValue - (bestValue + Math.log(expSum || 1)) = -Math.log(...)
  }

  return { frameIds, selectedLogProbs };
}

// Optimized implementation with ! operator as per memory instructions
function argmaxAndSelectedLogProbsMemoryRule(logits, frameCount, vocabSize) {
  const frameIds = new Array(frameCount).fill(0);
  const selectedLogProbs = new Float32Array(frameCount);
  ...
  for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
    ...
    for (let vocabIndex = 0; vocabIndex < vocabSize; vocabIndex += 1) {
      // Using non-null assertion as memory suggests
      const value = logits[rowOffset + vocabIndex];
      // The `!` is a TS construct, in JS it just is accessing the element. The previous original used `?? Number.NEGATIVE_INFINITY`. We can just drop it if we know we won't get undefined.
      if (value > bestValue) {
        bestValue = value;
        bestId = vocabIndex;
      }
...
function runBench() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    argmaxAndSelectedLogProbsMemoryRule(logits, frameCount, vocabSize);
  }
  ...
  console.log(`MemoryRule: ${(endMemory - startMemory).toFixed(2)}ms`);
}
