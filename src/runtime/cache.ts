import { normalizePcmInput, type PcmAudioBuffer } from '../processors/index.js';
...
export function createAudioCacheKey(input: AudioInputLike): string {
  ...
  const mix32 = (state: number, sample: number): number => {
    const quantized = Math.max(-32768, Math.min(32767, Math.round((Number.isFinite(sample) ? sample : 0) * 32768)));
...
export class AudioFeatureCache<TValue> {
  ...
  constructor(options: AudioFeatureCacheOptions<TValue> = {}) {
    ...
    this.estimateSizeBytes = options.estimateSizeBytes ?? ((value) => defaultEstimateSizeBytes(value));
    this.keyResolver = options.keyResolver ?? ((audio) => createAudioCacheKey(audio));
  }

  has(input: AudioInputLike | string): boolean {
    const key = typeof input === 'string' ? input : this.keyResolver(normalizePcmInput(input).toMono());
