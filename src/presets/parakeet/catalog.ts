...
// ⚡ Bolt: Cache Set<string> for O(1) language checks
const LANGUAGE_SET_CACHE = new WeakMap<ParakeetModelConfig, Set<string>>();
...
export function supportsLanguage(modelKeyOrRepoId: string, language: string): boolean {
  ...
  // ⚡ Bolt: Replace O(N) array.includes with O(1) Set lookup
  let langSet = LANGUAGE_SET_CACHE.get(config);
  if (!langSet) {
    langSet = new Set(config.languages);
    LANGUAGE_SET_CACHE.set(config, langSet);
  }
