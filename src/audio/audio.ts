...
export class PcmAudioBuffer implements AudioBufferLike {
  ...
  toChunk(options: Omit<Partial<AudioChunkInit>, 'sampleRate' | 'channels'> = {}): AudioChunk {
...
export class AudioChunk extends PcmAudioBuffer implements AudioChunkLike {
  ...
  constructor(init: AudioChunkInit) {
    ...
    this.endTimeSeconds =
      init.endTimeSeconds ??
      (init.startTimeSeconds !== undefined
        ? init.startTimeSeconds + this.durationSeconds
        : undefined);
...
export function normalizePcmInput(
  input: AudioInputLike,
  options: NormalizePcmOptions = {},
): PcmAudioBuffer {
  ...
  throw new TypeError(
    'normalizePcmInput expected mono PCM or an AudioBufferLike with channel data.',
  );
}

export function normalizePcmChunk(
  input: AudioInputLike,
  options: NormalizePcmOptions = {},
): AudioChunk {
...
export function framePcmAudio(
  ...
  for (
    ...
    start < audio.numberOfFrames;
    start += hopLengthFrames, sequence += 1
  ) {
...
export function chunkPcmAudio(
  ...
  if (overlapFrames < 0 || overlapFrames >= chunkLengthFrames) {
    throw new RangeError(
      'chunkPcmAudio expects overlapFrames to be >= 0 and smaller than chunkLengthFrames.',
    );
  }
