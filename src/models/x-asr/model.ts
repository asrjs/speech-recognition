...
const DEFAULT_GRAPH = { encoderStateInputs: [], encoderFrameSize: 16, encoderFrameShift: 16, decoderContextSize: 2 } as const;
...
function canonical(native: XAsrNativeTranscript, modelId: string, backendId: string, sampleRate: number, durationSeconds: number, detail: XAsrTranscriptionOptions['detail']) {
  return mapLasrCtcNativeToCanonical(native, CLASSIFICATION, { detailLevel: detail, backendId, modelId, sampleRate, durationSeconds, language: 'zh-en', metrics: native.metrics });
}
...
class XAsrStreamingTranscriber implements StreamingTranscriber {
  ...
  async finalize(): Promise<PartialTranscript> { this.assertOpen(); const result = await this.executor.pushStream(this.stream, new Float32Array(0), true, { detail: this.detail }); this.finalized = true; return this.update(result, 'final'); }
  async reset(): Promise<void> { this.assertOpen(); this.stream = this.executor.createStream(); this.accumulator.reset(); this.durationSeconds = 0; this.revision = 0; }
