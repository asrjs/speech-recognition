...
import type { XAsrNativeTranscript, XAsrTranscriptionOptions } from './types.js';

/** Maps X-ASR's stateful transducer output without importing a CTC family mapper. */
export function mapXAsrNativeToCanonical(
  nativeTranscript: XAsrNativeTranscript,
  classification: ModelClassification,
  meta: Omit<TranscriptMeta, 'detailLevel' | 'isFinal'> & {
    readonly detailLevel: XAsrTranscriptionOptions['detail'];
  },
): TranscriptResult {
  ...
  const result: TranscriptResult = {
    ...
    meta: {
      ...
      modelFamily: classification.family ?? 'x-asr',
