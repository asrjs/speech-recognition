...
class GigaAmSession implements GigaAmBatchSession {
  ...
  constructor(
    ...
    private readonly executor: OrtGigaAmCtcExecutor,
  ...
  async transcribe<TFlavor extends TranscriptResponseFlavor = 'canonical'>(
    ...
    const canonical = mapLasrCtcNativeToCanonical(native, CLASSIFICATION, {
    ...
    if (flavor === 'canonical+native')
      return { canonical, native } as TranscriptResponse<LasrCtcNativeTranscript, TFlavor>;
  ...
  async transcribeBatch<TFlavor extends TranscriptResponseFlavor = 'canonical'>(
    ...
    if (nativeTranscripts.length !== audios.length) {
      throw new Error(
        `GigaAM CTC batch returned ${nativeTranscripts.length} results for ${audios.length} inputs.`,
      );
    }
    ...
    return nativeTranscripts.map((native, index) => {
      ...
      if (responseFlavor === 'native') {
        return native as TranscriptResponse<LasrCtcNativeTranscript, TFlavor>;
      }
...
class GigaAmModel implements SpeechModel<
  GigaAmModelOptions,
...
> {
  ...
  constructor(
    ...
    readonly config: GigaAmModelConfig,
    loadOptions: GigaAmModelOptions | undefined,
    private readonly dependencies: NonNullable<GigaAmModelFamilyOptions['dependencies']>,
  ) {
    ...
    this.info = {
      ...
      description: `GigaAM Multilingual character CTC model for ${modelId}.`,
  ...
  async createSession(_options: BaseSessionOptions = {}): Promise<GigaAmBatchSession> {
    const executor =
      ...
      new OrtGigaAmCtcExecutor(this.modelId, this.backend.id, this.config, this.loadOptions, {
        assetProvider: this.dependencies.assetProvider,
        runtimeHooks: this.dependencies.runtimeHooks,
        signal: this.dependencies.signal,
      });
    const session = new GigaAmSession(this.modelId, this.backend.id, executor, () =>
...
export function createGigaAmCtcModelFamily(
  options: GigaAmModelFamilyOptions = {},
): SpeechModelFactory<GigaAmModelOptions, LasrCtcTranscriptionOptions, LasrCtcNativeTranscript> {
  return {
    family: 'gigaam-ctc',
    classification: CLASSIFICATION,
    supports(modelId: string): boolean {
      const value = modelId.toLowerCase();
      return value.includes('gigaam') && value.includes('ctc');
    },
    matchesClassification(classification): boolean {
    ...
    async createModel(request, context) {
      ...
      return new GigaAmModel(
        context.backend,
        'gigaam-ctc',
        request.modelId,
        { ...DEFAULT_CONFIG, ...(request.options?.config ?? {}) },
        request.options,
        dependencies,
      );
    },
