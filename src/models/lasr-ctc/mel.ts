...
function createMelFilterbank(
  ...
  for (let index = 0; index < melPoints - 1; index += 1) {
    ...
    const next = melFrequencies[index + 1]!;
  ...
  for (let melIndex = 0; melIndex < nMels; melIndex += 1) {
    ...
    const centerRight = melFrequencies[melIndex + 2]!;
    const lowerDelta = melDifferences[melIndex]!;
    const upperDelta = melDifferences[melIndex + 1]!;
  ...
  for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
    for (let melIndex = 0; melIndex < nMels; melIndex += 1) {
      transposed[frameIndex * nMels + melIndex] =
        featuresMxT[melIndex * frameCount + frameIndex]!;
    }
  }
      ...
      for (let melIndex = 0; melIndex < this.nMels; melIndex += 1) {
        ...
        for (let frequencyIndex = lower; frequencyIndex < upper; frequencyIndex += 1) {
          melValue +=
            ...
            (this.melFilterbank[melOffset + frequencyIndex]!);
        }
