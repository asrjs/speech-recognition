  ...
  async transcribe(input: AudioBufferLike, options: GigaAmRnntTranscriptionOptions = {}): Promise<GigaAmRnntNativeTranscript> {
    ...
    for (let frame = 0; frame < frames; frame += 1) {
      ...
      while (emitted < this.config.maxTokensPerFrame) {
        ...
        decoderOutput = firstOutput(decoderOutputs, 'dec'); const nextH = tensorData(firstOutput(decoderOutputs, 'h')); const nextC = tensorData(firstOutput(decoderOutputs, 'c')); h.set(nextH.subarray(0, h.length)); c.set(nextC.subarray(0, c.length));
        const encFrame = new Float32Array(hidden); for (let i = 0; i < hidden; i += 1) encFrame[i] = encData[i * (encDims[2] ?? frames) + frame] ?? 0;
        const encTensor = tensor(state.ort, 'float32', encFrame, [1, hidden, 1]); const decData = tensorData(decoderOutput); const decTensor = tensor(state.ort, 'float32', decData, [1, decData.length, 1]); let jointOutputs: Record<string, OrtTensorLike>; try { jointOutputs = await state.joint.run({ enc: encTensor, dec: decTensor }); } finally { encTensor.dispose?.(); decTensor.dispose?.(); }
