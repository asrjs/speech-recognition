#!/usr/bin/env node
/**
 * Whisper splitgraph variant validation — V2 (session reuse).
 *
 * Identical CLI and output to V1 but creates ONNX sessions once per variant
 * instead of per fixture. Reduces session count from N*V*3 to V*3.
 */
...
function _checkTimestampsSuppressed(logits, timestampBegin) {
...
function makeSessionOptions(runtimeBackend, variantDir, manifestRaw, graphName) {
  if (runtimeBackend === 'node-cpu') {
    return {
  ...
  if (opts.externalData?.length === 0) delete opts.externalData;
  return opts;
}

async function createSessions(ort, variantDir, manifestRaw, runtimeBackend) {
  const t0 = performance.now();
  const encoder = await ort.InferenceSession.create(
    path.join(variantDir, GRAPH_FILES.encoder),
    makeSessionOptions(runtimeBackend, variantDir, manifestRaw, 'encoder'),
  );
  const decoderInit = await ort.InferenceSession.create(
    path.join(variantDir, GRAPH_FILES.decoder_init),
    makeSessionOptions(runtimeBackend, variantDir, manifestRaw, 'decoder_init'),
  );
  const decoderStep = await ort.InferenceSession.create(
    path.join(variantDir, GRAPH_FILES.decoder_step),
    makeSessionOptions(runtimeBackend, variantDir, manifestRaw, 'decoder_step'),
  );
  let decoderAlign = null;
  if (fs.existsSync(path.join(variantDir, GRAPH_FILES.decoder_align))) {
    decoderAlign = await ort.InferenceSession.create(
      path.join(variantDir, GRAPH_FILES.decoder_align),
      makeSessionOptions(runtimeBackend, variantDir, manifestRaw, 'decoder_align'),
    );
  }
  const sessionMs = performance.now() - t0;
  return { encoder, decoderInit, decoderStep, decoderAlign, sessionMs };
}

function releaseSessions(sessions) {
  for (const s of [sessions.encoder, sessions.decoderInit, sessions.decoderStep, sessions.decoderAlign]) {
...
// ---------------------------------------------------------------------------
// Fixture inference — reuses pre-created sessions
// ---------------------------------------------------------------------------
...
async function runFixture(sessions, variantState, fixture, maxNewTokens, enableAlign, _debugBaseline = null) {
  ...
  const eosId = variantState.tokenizer.getTokenId('') ?? 50257;
  ...
  // --- V2: reuse sessions, no create/release per fixture ---
  const t0 = performance.now();

  // Encoder run (session reused)
  const encOuts = await sessions.encoder.run({ input_features: encoderInput });
  const enc = encOuts[Object.keys(encOuts)[0]];
  const encoderDims = Array.from(enc.dims ?? []);

  // Decoder init (session reused)
  const promptTensor = makeTensor(ort, 'int64', new BigInt64Array(promptIds.map(BigInt)), [1, promptIds.length]);
  const initOut = await sessions.decoderInit.run({ input_ids: promptTensor, encoder_hidden_states: enc });
  ...
  if (debug && _debugBaseline) {
    ...
    debug.steps.push({
      ...
      timestamp_tokens_suppressed: _checkTimestampsSuppressed(firstLogits, timestampBegin),
  ...
  // Decoder step loop (session reused)
  for (let step = 1; step < maxNewTokens && !eosReached; step++) {
    ...
    const stepOut = await sessions.decoderStep.run(feeds);
    ...
    if (debug && _debugBaseline) {
      ...
      if (mismatch) {
        const stepInfo = {
          ...
          timestamp_tokens_suppressed: _checkTimestampsSuppressed(logits, timestampBegin),
          suppress_tokens_active: controls.suppress_tokens?.length ?? 0,
          begin_suppress_tokens_active: controls.begin_suppress_tokens?.length ?? 0,
  ...
  let alignment = null;
  if (enableAlign && sessions.decoderAlign && tokens.length > 0) {
    const allTokens = [...promptIds, ...tokens];
    const alignOut = await sessions.decoderAlign.run({
...
function generateReport({ modelDir, fixtures, results, maxNewTokens, align, reportPath }) {
  ...
  lines.push('**Validator**: V2 (session reuse per variant)');
...
// ---------------------------------------------------------------------------
// Main — V2: create sessions per variant, reuse across fixtures
// ---------------------------------------------------------------------------
...
async function main() {
  ...
  if (args.debugDivergence) {
    ...
    // Create sessions once per variant
    console.log('\n== Creating fp32 sessions ==');
    const fp32Sessions = await createSessions(fp32State.ort, fp32State.variantDir, fp32State.manifestRaw, fp32State.runtimeBackend);
    console.log(`  Session creation: ${(fp32Sessions.sessionMs / 1000).toFixed(1)}s`);

    console.log('\n== Creating q8 sessions ==');
    const q8Sessions = await createSessions(q8State.ort, q8State.variantDir, q8State.manifestRaw, q8State.runtimeBackend);
    console.log(`  Session creation: ${(q8Sessions.sessionMs / 1000).toFixed(1)}s`);

    const baselineResults = [];
    const q8Results = [];
    for (const fixture of fixtures) {
      console.log(`\n--- fp32 baseline: ${fixture.filename} ---`);
      const fp32Result = await runFixture(fp32Sessions, fp32State, fixture, args.maxNewTokens, false, null);
      ...
      const q8Result = await runFixture(q8Sessions, q8State, fixture, args.maxNewTokens, false, fp32Result.tokens);
    ...
    releaseSessions(fp32Sessions);
    releaseSessions(q8Sessions);
  ...
  for (const variant of args.variants) {
    console.log(`\n== ${variant} ==`);
    const state = await loadVariant(ortWasm, modelDir, variant);
    console.log(`  Loading variant config: ${(state.loadMs / 1000).toFixed(3)}s`);

    const tSessionStart = performance.now();
    const sessions = await createSessions(state.ort, state.variantDir, state.manifestRaw, state.runtimeBackend);
    console.log(`  Session creation: ${(sessions.sessionMs / 1000).toFixed(1)}s (${((performance.now() - tSessionStart) / 1000).toFixed(1)}s wall)`);

    const variantResult = { variant, load_sec: Number((state.loadMs / 1000).toFixed(3)), session_sec: Number((sessions.sessionMs / 1000).toFixed(1)), fixtures: [] };
    for (const fixture of fixtures) {
      console.log(`  ${fixture.filename} (${fixture.language})`);
      variantResult.fixtures.push(await runFixture(sessions, state, fixture, args.maxNewTokens, args.align));
    }
