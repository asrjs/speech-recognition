import { NemoConformerForTDT, Tensor } from "../../../src/reference-runtime.js";
...
import { MODEL_CLASS_TYPE_MAPPING } from "../../../src/models/registry.js";
...
export default () => {
  describe("NemoConformerForTDT", () => {
    ...
    it("registers nemo-conformer-tdt under the encoder-decoder base mapping", () => {
      const encoderOnlyMapping = MODEL_CLASS_TYPE_MAPPING.find(([, type]) => type === MODEL_TYPES.EncoderOnly)?.[0];
      const encoderDecoderMapping = MODEL_CLASS_TYPE_MAPPING.find(([, type]) => type === MODEL_TYPES.EncoderDecoder)?.[0];

      expect(encoderOnlyMapping?.has("nemo-conformer-tdt")).toBe(false);
      expect(encoderDecoderMapping?.get("nemo-conformer-tdt")).toBe("NemoConformerForTDT");
    });
    ...
    it(
      ...
      async () => {
        const invalidConfig = {
          ...BASE_CONFIG,
          "reference_js_config": {
            ...BASE_CONFIG["reference_js_config"],
            transducer: {
              ...BASE_CONFIG["reference_js_config"].transducer,
              vocab_size: 0,
            },
          },
        };
    ...
    it("fails fast when transducer config is missing", () => {
      ...
      expect(() => new NemoConformerForTDT(invalidConfig, BASE_SESSIONS, {})).toThrow("Missing `reference_js_config.transducer`");
    });
